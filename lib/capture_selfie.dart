...
class _SelfieCaptureScreenState extends State<SelfieCaptureScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    // Don't dispose on 'inactive' — iOS shows permission sheet under this state.
    if (state == AppLifecycleState.paused || state == AppLifecycleState.detached) {
      ...
      // Small delay avoids "camera in use by another app" on some devices
  ...
  Future<void> _initCamera() async {
    ...
    try {
      ...
      // Always tear down previous safely
      try {
        await _controller?.dispose();
      } catch (_) {}
      final controller = CameraController(
        front,
        ResolutionPreset.medium, // lower pressure, same UI
        enableAudio: false,
        imageFormatGroup: ImageFormatGroup.jpeg,
      );
      _controller = controller;

      // Some devices need a timeout + a second attempt right after permission grant
      Future<void> _doInit() =>
          controller.initialize().timeout(const Duration(seconds: 8));
      try {
        await _doInit();
      } on TimeoutException {
        await _doInit(); // retry once
      }
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snap) {
          ...
          if (_controller == null || !_controller!.value.isInitialized) {
            // auto-retry once if not already trying
            if (!_initInFlight) {
              _ready = _initCamera();
            }
            return const Center(
...
class _CircleMaskPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Keep same visual (solid orange ring). Using color instead of a 1-stop gradient.
...
Future<void> _initCamera() async {
  ...
  try {
    ...
    // Some devices need a timeout + a second attempt right after permission grant
    Future<void> _doInit() => controller.initialize().timeout(const Duration(seconds: 8));
    try {
      await _doInit();
    } on TimeoutException {
      // retry once
    ...
    if (!mounted) {
      // if widget went away mid-init, clean up
      await controller.dispose();
      return;
    }
    ...
    final msg = switch (e.code) {
      'CameraAccessDenied' || 'cameraPermission' => 'Camera permission denied',
      'CameraAccessDeniedWithoutPrompt' => 'Camera permission disabled in Settings',
      _ => e.description ?? e.code,
    };
    debugPrint("Camera init error: $e");
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Camera init error: $msg')));
    }
  } catch (e) {
    debugPrint("Camera init error: $e");
    if (mounted) {
