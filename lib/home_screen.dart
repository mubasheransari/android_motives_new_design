...
class _HomeUpdatedState extends State<HomeUpdated> {
  ...
  // for Smart FAQ FAB animation
  double _faqBtnScale = 1.0;

  // 🔑 Keys for GetStorage flags (same as MarkAttendanceView / RouteScreen)
  ...
  static const String kRouteKey = 'routeKey';
  ...
  void initState() {
    ...
    // ✅ keep attendance flag in sync with date (same logic as MarkAttendanceView)
    final todayKey = _todayKey();
    final storedDate = box.read<String>(kAttendanceDateKey);
    if (storedDate != todayKey) {
      box.write(kAttendanceDateKey, todayKey);
      box.write(kAttendanceFlagKey, false);
    }

    // seed current values
    _routeStarted = box.read(kRouteKey) == true;
    _coveredRoutesCount = _asInt(box.read('covered_routes_count'));

    // live updates
    box.listenKey(kRouteKey, (v) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    // ── Counts / flags ─────────────────────────────────────────────
    final jpCount =
        dedupJourneyCount(context.read<GlobalBloc>().state.loginModel!.journeyPlan);

    // NEW: attendance + route lock from local storage
    final todayKey = _todayKey();
    final bool attendanceMarkedToday = box.read(kAttendanceFlagKey) == true;
    final String? lockedDate = box.read<String>(kRouteLockKey);
    final bool routeLockedToday = lockedDate == todayKey;

    // Once route is locked for today, all related actions (Attendance, Routes, Punch Order)
    // are disabled.
    final bool dayLocked = routeLockedToday;
            ...
            child: GestureDetector(
              ...
              onTapUp: (_) {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const SmartFaqChatBilingual(),
                  ),
                );
              },
              child: Stack(
            ...
            SafeArea(
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  // ✅ Hide stats when today is locked (route ended & locked)
                  if (!dayLocked && attendanceMarkedToday == true)
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: _MiniStatCard(
                                title: 'Done $attendanceMarkedToday',
                  ...
                  // ✅ Banner only when today is locked
                  if (dayLocked)
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Center(
                          child: Text(
                            "Route ended. Today’s attendance, route and punch order are locked.",
                            textAlign: TextAlign.center,
                      ...
                      delegate: SliverChildListDelegate.fixed([
                        ...
                        _Enter(
                          ...
                          child: _GlassActionCard(
                            ...
                            onTap: () {
                              if (dayLocked) {
                                toastWidget(
                                  'Today is locked. You cannot mark or review attendance now.',
                                  Colors.red,
                                );
                                return;
                              }
                        ...
                        _Enter(
                          ...
                          child: _GlassActionCard(
                            ...
                            subtitle: 'Punch Order',
                            onTap: () {
                              if (dayLocked) {
                                toastWidget(
                                  'Today’s route is locked. Punch Order is disabled.',
                                  Colors.red,
                                );
                                return;
                              }
                              ...
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => const JourneyPlanScreen(),
                                ),
                              );
                            },
                          ),
...
class _GlassActionCard extends StatelessWidget {
  ...
  final IconData icon;
  final String title;    // small
  final String subtitle; // big
...
class _HomeUpdatedState extends State<HomeUpdated> {
  ...
  Widget build(BuildContext context) {
    ...
    // original route completed logic
    final routeCompleted = (jpCount > 0 && jpCount == _coveredRoutesCount);

    // attendance status from backend (for today)
    final hasTodayAttendance =
        state.loginModel?.statusAttendance.toString() == "1";

    // 👉 Once attendance is marked AND route is completed => lock for today
    final bool dayLocked = hasTodayAttendance && routeCompleted;
            ...
            SafeArea(
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  // ✅ Hide stats when today is locked (attendance + route completed)
                  if (!dayLocked)
                    SliverToBoxAdapter(
                      child: Padding(
                        padding: const EdgeInsets.fromLTRB(20, 24, 20, 0),
                  ...
                  if (dayLocked)
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Center(
                          child: Text(
                            "Route completed! Today’s attendance and route are closed.",
                            textAlign: TextAlign.center,
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                                  color: HomeUpdated.cPrimary,
                                  fontWeight: FontWeight.w800,
                        ...
                        _Enter(
                          ...
                          child: _GlassActionCard(
                            ...
                            onTap: () {
                              if (dayLocked) {
                                toastWidget(
                                  'Today’s attendance and route are already completed.',
                                  Colors.red,
                                );
                                return;
                              }
...
//                   if (routeCompleted != true  //state.loginModel?.statusAttendance.toString() == "1"
//                   )
//                     SliverToBoxAdapter(
//                       child: Padding(
...
//                     if (routeCompleted == true && _routeStarted == true)
//                     SliverToBoxAdapter(
//                       child: Padding(
//                         padding: const EdgeInsets.fromLTRB(20, 8, 20, 0),
//                         child: Center(
