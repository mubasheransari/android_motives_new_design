...
class InvoicesScreen extends StatefulWidget {
  final String? acode; // required by you (kept nullable for call-site flexibility)
  final String? disid;
...
class _InvoicesScreenState extends State<InvoicesScreen> {
  ...
  double _toNum(String? s) => double.tryParse((s ?? '').replaceAll(',', '')) ?? 0.0;
            ...
            child: BlocBuilder<GlobalBloc, GlobalState>(
              ...
              builder: (context, state) {
                ...
                final data = q.isEmpty
                    ? all
                    : all.where((m) {
                        final invNo = (m.invno ?? '').toLowerCase();
                        final ac = (m.acode ?? '').toLowerCase();
                        return invNo.contains(q) || ac.contains(q);
                      }).toList();
                ...
                return RefreshIndicator(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      // map exact fields you provided
                      final invid      = m.invid;        // "234178"
                      final cashCredit = m.cashCredit;   // "CR"
                      final invno      = m.invno;        // "110842000001"
                      final invDate    = m.invDate;      // "16-DEC-19"
                      final acode      = m.acode;        // "500001"
                      final invAmount  = _toNum(m.invAmount); // "15332.63"
                      final ledAmount  = _toNum(m.ledAmount); // "0"
                      final balAmount  = _toNum(m.balAmount); // "15332.63"
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          leading: CircleAvatar(
                            child: Text((cashCredit ?? '').toUpperCase()),
                          ),
                          // show invoice number prominently
                          title: Text('Invoice #${invno ?? '-'}'),
                          // show date + acode + invid
                          subtitle: Text([
                            if ((invDate ?? '').isNotEmpty) _fmtDate(invDate),
                            if ((acode ?? '').isNotEmpty) 'A/C $acode',
                            if ((invid ?? '').isNotEmpty) 'ID $invid',
                          ].join('   •   ')),
                          // amounts column
                          trailing: Column(
                            ...
                            children: [
                              Text('Amount: ${invAmount.toStringAsFixed(2)}'),
                              Text('Ledger: ${ledAmount.toStringAsFixed(2)}',
                                  style: const TextStyle(fontSize: 12)),
                              Text('Balance: ${balAmount.toStringAsFixed(2)}',
