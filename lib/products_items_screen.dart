...
//   final String desc;
//   final String brand;
//   final int ctnSize; // packs per carton (best-effort, from item.packQty/ctnQty)
...
//     // pack_qty or ctn_qty in your Item; pick whichever is set
//     final ctnSize = int.tryParse(
//           ((i.packQty ?? i.ctnQty ?? '').toString())
//               .replaceAll(RegExp(r'[^0-9]'), ''),
//         ) ??
...
//   if (!ok) {
//     // enqueue + store as queued with NEW fields
//     await SyncService.instance.enqueueOrder(
//       endpoint: endpoint,
//       payload: legacyPayload,
//       requestField: requestField,
...
//   // SUCCESS path — store with NEW fields:
//   final rec = OrderRecord(
//     id: orderId,
//     userId: userId,
//     distId: distId,
...
//           _key(userId, shopId),
//           jsonEncode(
//               Map.of(cart)..removeWhere((_, q) => q <= 0)));

//   Future<void> clear(String? userId, String? shopId) async =>
//       _box.remove(_key(userId, shopId));
// }
...
// /// Build payload from separate SKU / CTN carts, persisting client-only fields.
// /// Each line adds: _client_item_name, _client_ctn_size (if > 0).
// /// Header adds: _client_shop_name (if provided).
// Map<String, dynamic> buildLegacyOrderPayloadFromTea({
//   required List<TeaItem> allItems,
//   required Map<String, int> cartSku,
//   required Map<String, int> cartCtn,
...
//   // two carts: SKU persisted via CartStorage; CTN via GetStorage key
//   final Map<String, int> _cartSku = {};
//   final Map<String, int> _cartCtn = {};
...
//       final login = context.read<GlobalBloc>().state.loginModel;
//       _activeUserId = login?.userinfo?.userId;
//       _activeDistributorId = (login?.distributors.isNotEmpty ?? false)
...
//   Future<void> _persistSku() =>
//       _skuStorage.save(_cartSku, _activeUserId, _activeShopId);

//   int _getSku(String k) => _cartSku[k] ?? 0;
//   int _getCtn(String k) => _cartCtn[k] ?? 0;

//   void _incSku(TeaItem item) {
//     setState(() => _cartSku[item.key] = _getSku(item.key) + 1);
//     _persistSku();
//   }
...
//       final q = _getSku(item.key);
//       if (q > 1) {
//         _cartSku[item.key] = q - 1;
//       } else {
//         _cartSku.remove(item.key);
//       }
//     });
//     _persistSku();
//   }

//   void _incCtn(TeaItem item) {
//     setState(() => _cartCtn[item.key] = _getCtn(item.key) + 1);
//     _saveCtnMap();
//   }
...
//       final q = _getCtn(item.key);
//       if (q > 1) {
//         _cartCtn[item.key] = q - 1;
//       } else {
//         _cartCtn.remove(item.key);
//       }
//     });
//     _saveCtnMap();
//   }
...
//     final login = context.read<GlobalBloc>().state.loginModel;
//     final userId = login?.userinfo?.userId;
//     final distributorId = (login?.distributors.isNotEmpty ?? false)
//         ? login!.distributors.first.id
//         : null;
...
//     final lines = <String>[
//       "All",
//       ...{for (final i in items) i.brand}.where((s) => s.isNotEmpty)
//     ];
//     final q = _search.text.trim().toLowerCase();
//     final filtered = items.where((e) {
//       final lineOk = _selectedLine == "All" || e.brand == _selectedLine;
...
//     String qtyLabel() {
//       if (totalSku > 0 && totalCtn > 0) {
//         return 'SKU: $totalSku • CTN: $totalCtn';
//       }
//       if (totalCtn > 0) return 'CTN: $totalCtn';
//       return 'SKU: $totalSku';
//     }
...
//                     child: _QtyModeTile(
//                         label: 'SKU',
//                         selected: _qtyMode == QtyMode.sku,
//                         onTap: () => setState(() => _qtyMode = QtyMode.sku))),
//                 const SizedBox(width: 8),
//                 Expanded(
//                     child: _QtyModeTile(
//                         label: 'CTN',
//                         selected: _qtyMode == QtyMode.ctn,
//                         onTap: () => setState(() => _qtyMode = QtyMode.ctn))),
//                 const SizedBox(width: 8),
//                 Expanded(
//                     child: _QtyModeTile(
//                         label: 'Both',
//                         selected: _qtyMode == QtyMode.both,
//                         onTap: () => setState(() => _qtyMode = QtyMode.both))),
//               ],
//             ),
//           ),
...
//                 const Spacer(),
//                 if (totalAll > 0)
//                   Text('In list: ${qtyLabel()}',
//                       style: t.bodySmall
//                           ?.copyWith(color: kOrange, fontWeight: FontWeight.w700)),
//               ],
//             ),
//           ),
...
//                   name: item.name,
//                   desc: item.ctnSize > 0
//                       ? '${item.desc} • ${item.ctnSize} per CTN'
//                       : item.desc,
//                   brand: item.brand,
//                   qtySku: _getSku(item.key),
//                   qtyCtn: _getCtn(item.key),
...
// // ===== PRODUCT CARD (supports SKU/CTN/Both) =====
// class _ProductCard extends StatelessWidget {
//   final String name;
//   final String desc;
//   final String brand;
...
//       switch (mode) {
//         case QtyMode.sku:
//           return _QtyControlsSingle(
//               label: 'SKU', qty: qtySku, onInc: onIncSku, onDec: onDecSku);
//         case QtyMode.ctn:
//           return _QtyControlsSingle(
//               label: 'CTN', qty: qtyCtn, onInc: onIncCtn, onDec: onDecCtn);
//         case QtyMode.both:
//           return Row(
//             mainAxisSize: MainAxisSize.min,
//             children: [
...
//                   onPressed: onDec,
//                   icon:
//                       const Icon(Icons.remove_rounded, size: 20, color: kText)),
//               Padding(
//                 padding: const EdgeInsets.symmetric(horizontal: 8),
//                 child: Text('$qty',
//                     style: const TextStyle(
...
//           borderRadius: BorderRadius.circular(10),
//           border: Border.all(
//               color: selected ? kOrange : const Color(0xFFE5E7EB)),
//         ),
//         child: Text(label,
//             style: TextStyle(
//                 fontWeight: FontWeight.w700,
...
//   List<_CartRow> get _rows {
//     final keys =
//         <String>{...widget.cartSku.keys, ...widget.cartCtn.keys}.toList()
//           ..sort();
//     final rows = <_CartRow>[];
//     for (final k in keys) {
//       final item = widget.allItems.firstWhere(
...
//       rows.add(_CartRow(
//           item: item,
//           sku: widget.cartSku[k] ?? 0,
//           ctn: widget.cartCtn[k] ?? 0));
//     }
//     rows.sort((a, b) => a.item.name.compareTo(b.item.name));
//     return rows;
//   }
...
//     // --- NEW: find party_name & cust_address from JourneyPlan ---
//     JourneyPlan? shop;
//     try {
//       shop = login?.journeyPlan.firstWhere((p) => p.accode == accCode);
//     } catch (_) { shop = null; }
...
//     // --- NEW: collect product names from your cart snapshot ---
//     final keys = <String>{...widget.cartSku.keys, ...widget.cartCtn.keys}.toList()..sort();
//     final names = <String>[];
//     for (final k in keys) {
//       final sku = widget.cartSku[k] ?? 0;
...
//       final item = widget.allItems.firstWhere(
//         (e) => e.key == k,
//         orElse: () => TeaItem(key: k, itemId: null, name: 'Unknown', desc: '', brand: 'Meezan'),
//       );
//       if (item.name.isNotEmpty && item.name != 'Unknown') {
//         names.add(item.name);
//       }
//     }
...
//       headerOrderType: 'OR',
//       orderStatus: 'N',
//       shopName: partyName,           // NEW: echo into payload (client-only)
//       shopAddress: custAddress,      // NEW: echo into payload (client-only)
//       deviceId: Platform.isAndroid
//           ? 'android-${DateTime.now().millisecondsSinceEpoch}'
//           : 'ios-${DateTime.now().millisecondsSinceEpoch}',
//       appSource: 'flutter.motives',
...
//       // NEW: persist these into OrderRecord:
//       shopName: partyName,
//       shopAddress: custAddress,
//       itemNames: names,
//     );
...
//     // ... (rest of your success logic unchanged: events, journey flags, clear carts)
//     // ...
//   } finally {
//     if (mounted) setState(() => _isSubmitting = false);
//   }
...
//         if (!mounted) return;
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//             content: Text('User/Distributor not found ($userId/$distId)')));
//         return;
//       }
...
//         compId: (compId).trim().isNotEmpty ? compId.trim() : '11',
//         orderBookerId:
//             orderBookerId.isNotEmpty ? orderBookerId : userId,
//         paymentType: widget.creditLimit != "0" ? _paymentType : "CS",
//         headerOrderType: 'OR',
//         orderStatus: 'N',
//         shopName: null, // pass a party name if you have it in UI
//         deviceId: Platform.isAndroid
//             ? 'android-${DateTime.now().millisecondsSinceEpoch}'
//             : 'ios-${DateTime.now().millisecondsSinceEpoch}',
//         appSource: 'flutter.motives',
...
//                               qty: row.sku,
//                               onInc: () {
//                                 widget.onIncrementSku(row.item);
//                                 setState(() {});
//                               },
//                               onDec: () {
//                                 widget.onDecrementSku(row.item);
//                                 setState(() {});
//                               },
//                             ),
//                             const SizedBox(height: 8),
...
//                               qty: row.ctn,
//                               onInc: () {
//                                 widget.onIncrementCtn(row.item);
//                                 setState(() {});
//                               },
//                               onDec: () {
//                                 widget.onDecrementCtn(row.item);
//                                 setState(() {});
//                               },
//                             ),
//                           ],
...
//                     style:
//                         TextStyle(fontWeight: FontWeight.w600, color: kText)),
//                 const SizedBox(height: 10),
//                 widget.creditLimit != "0"
//                     ? Row(
//                         children: [
...
//                     ),
//                     onPressed:
//                         _rows.isEmpty || _isSubmitting ? null : _submitOrder,
//                     child: _isSubmitting
//                         ? const SizedBox(
//                             width: 22,
//                             height: 22,
...
//         Container(
//           decoration:
//               BoxDecoration(color: kField, borderRadius: BorderRadius.circular(10)),
//           child: Row(
//             mainAxisSize: MainAxisSize.min,
//             children: [
//               IconButton(
...
//                   onPressed: onDec,
//                   icon:
//                       const Icon(Icons.remove_rounded, size: 18, color: kText)),
//               Padding(
//                 padding: const EdgeInsets.symmetric(horizontal: 6),
//                 child: Text('$qty',
//                     style: const TextStyle(
...
//           boxShadow: const [
//             BoxShadow(color: kShadow, blurRadius: 10, offset: Offset(0, 4))
//           ],
//         ),
//         child: Row(
//           mainAxisAlignment: MainAxisAlignment.center,
...
List<TeaItem> mapItemsToTea(List<Item> raw) {
  return raw.asMap().entries.map((entry) {
    ...
    // pack_qty or ctn_qty in your Item; pick whichever is set
    final ctnSize = int.tryParse((i.packQty ?? i.ctnQty ?? '').replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
...
// ===== OPTIONAL SIMPLE CART REQUEST (unchanged helper) =====
class CartLineDto {
  final String? itemId;
  final String name;
  final String brand;
...
// ===== OFFLINE/ONLINE SUBMIT (unchanged API helper with JSON toggle) =====
Future<OrderSubmitResult> sendCartToApi({
  required BuildContext context,
  required Map<String, dynamic> legacyPayload,
  required String endpoint,
...
Map<String, dynamic> buildLegacyOrderPayloadFromTea({
  required List<TeaItem> allItems,
  required Map<String, CartQty> cart,     // key -> sku/ctn
  required String userId,
  required String distId,
  required String accCode,
  required String segmentId,
  ...
  required String orderBookerId,
  required String paymentType,
  required String headerOrderType, // "OR"
  required String orderStatus,     // "N"
  ...
  // optional extras (kept from your original)
  String? appSource,
  String? deviceId,
}) {
  final orderLines = <Map<String, dynamic>>[];
  ...
  cart.forEach((key, q) {
    final it = allItems.firstWhereOrNull((e) => e.key == key) ??
        TeaItem(key: key, itemId: null, name: 'Unknown', desc: '', brand: 'Meezan');

    final id = (it.itemId ?? '').trim();
    if (id.isEmpty) return;

    final int sku = (qtyMode == QtyMode.ctn) ? 0 : q.sku;
    final int ctn = (qtyMode == QtyMode.sku) ? 0 : q.ctn;
    final double total = (sku + ctn).toDouble(); // legacy
    ...
    orderLines.add({
      ...
      "_client_item_name": it.name,
      if (it.ctnSize > 0) "_client_ctn_size": it.ctnSize,
    });
  });
...
/*
// ====== LEGACY ORDER PAYLOAD (now supports SKU + CTN) ======
Map<String, dynamic> buildLegacyOrderPayloadFromTea({
  required List<TeaItem> allItems,
  required Map<String, int> cartSku,
  required Map<String, int> cartCtn,
  ...
  required String compId,
  required String orderBookerId,
  required String paymentType, // CR / CS
  String headerOrderType = 'OR',
  String orderStatus = 'N',
  String appSource = 'flutter.motives',
  ...
  for (final k in keys) {
    ...
    orderLines.add({
      ...
      "item_qty_sku": "$sku",
      "item_qty_ctn": "$ctn",
      "item_total_qty": (sku + ctn).toString(), // if needed as string
      "item_name": item.name,
    });
  }
  ...
  return {
    "unique": _uuidv4(),
    "user_id": userId,
    "date": _ddMmmYyLower(DateTime.now()),
    "acc_code": accCode,
    "segment_id": segmentId,
    "compid": compId,
    "order_booker_id": orderBookerId,
...
// ===== MAIN CATALOG (now supports QtyMode + dual carts) =====
class MeezanTeaCatalog extends StatefulWidget {
...
class _MeezanTeaCatalogState extends State<MeezanTeaCatalog> {
  ...
  // two carts: SKU persisted via CartStorage; CTN saved with a new GetStorage key
  final Map<String, int> _cartSku = {};
  final Map<String, int> _cartCtn = {};
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final login = context.read<GlobalBloc>().state.loginModel;
      _activeUserId = login?.userinfo?.userId;
      _activeDistributorId = login?.distributors.isNotEmpty == true ? login!.distributors.first.id : null;
      _activeShopId = widget.shopId;
  ...
  Widget build(BuildContext context) {
    ...
    final login = context.read<GlobalBloc>().state.loginModel;
    final userId = login?.userinfo?.userId;
    final distributorId = login?.distributors.isNotEmpty == true ? login!.distributors.first.id : null;
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (totalAll > 0)
                  Text('In list: ${qtyLabel()}',
                      style: t.bodySmall?.copyWith(color: kOrange, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
...
// =====================
// _MyListView (Cart / Submit) — now shows SKU + CTN and submits both
// =====================
class _MyListView extends StatefulWidget {
  final List<TeaItem> allItems;
  ...
  final String shopId, creditLimit;
...
class _MyListViewState extends State<_MyListView> {
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      final userId = (login?.userinfo?.userId ?? '').trim();
      final distId = (login?.userinfo?.disid ?? '').toString();
      final segmentId = (login?.distributors.isNotEmpty == true ? (login!.distributors.first.segment ?? '') : (login?.userinfo?.segment ?? '')).trim();
      final compId = (login?.distributors.isNotEmpty == true ? (login!.distributors.first.compid ?? '') : (login?.userinfo?.compid ?? '')).trim();
      final orderBookerId = (login?.userinfo?.obid ?? '').trim();
      final accCode = widget.shopId.trim();

      if (userId.isEmpty || distId.isEmpty) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('User/Distributor not found ($userId/$distId)')));
        return;
      }
      ...
      final payload = buildLegacyOrderPayloadFromTea(
        ...
        orderBookerId: orderBookerId.isNotEmpty ? orderBookerId : userId,
        paymentType: widget.creditLimit != "0" ? _paymentType : "CS",
        deviceId: Platform.isAndroid ? 'android-${DateTime.now().millisecondsSinceEpoch}' : 'ios-${DateTime.now().millisecondsSinceEpoch}',
      );
      ...
      final msg = result.success ? (result.serverMessage ?? 'Order submitted successfully')
                                 : (result.serverMessage ?? 'Order submit failed');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
      ...
      gb.add(LoadShopInvoicesRequested(acode: widget.shopId, disid: distId));
      gb.add(CheckinCheckoutEvent(type: '7', userId: userId, lat: lat, lng: lng, act_type: 'ORDER', action: 'ORDER PLACED', misc: widget.shopId, dist_id: distId));
      gb.add(CheckinCheckoutEvent(type: '6', userId: userId, lat: lat, lng: lng, act_type: 'SHOP_CHECK', action: 'OUT', misc: widget.shopId, dist_id: distId));
...
class _CartRow {
  ...
  final int sku;
  final int ctn;
  _CartRow({required this.item, required this.sku, required this.ctn});
}
...
class TeaItem {
  ...
  final String desc;
  final String brand;
  final int ctnSize; // NEW: packs per carton (best-effort)
